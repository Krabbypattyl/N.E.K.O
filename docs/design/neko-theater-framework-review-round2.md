...
状态：审查本身为**只读**，未改代码。其中**延迟相关项已按问题2.140实施并取得实测前后对照**（见「实施结果」）；**2.141的演绎核对与冻结反例对照修正了2.140的时限结论**；**F3（普通回合提前演出下一幕时点的确定性检查）与W1（工坊固定旁白证人诊断）已实施并验证，其余F／W项与FR2-01／02仍为建议**。现行合同仍以[架构开发文档](./neko-theater-architecture.md)第 2、4、10、13 节为准；失败样本、回退口径与验证等级仍以[问题文档](./neko-theater-issues-and-solutions.md)对应条目为准。
...
**A1（最高优先，已有量化）复核时限与预算重定。** 冻结反例回收曲线：12秒17.3%、15秒38.5%、18秒50.0%、20秒55.8%、22秒63.5%、30秒75.0%，边际收益在18—22秒转平缓。建议争议时限**18秒**、整回合复核预算**20秒**：按实测构成（快检2.4＋争议≤18）复核上限约20.4秒，与现值20.788秒持平，而争议判定机会从38.5%升到50.0%。若要更接近原判定能力取22／24秒组合，尾部等待相应回升。风险低（超时仍保留初判）。验证：以18秒重跑冻结对照＋一次压测。

**A2 让复核预算真正约束整条链。** 现值只在"追加复检"前检查，导致本轮`review_budget_skips=0`、预算从未生效。改法：以`剩余预算＝预算−已用复核时间`作为**每次复核调用的时限上限**（`timeout=min(配置时限, 剩余预算)`），使复核阶段存在硬上界而不依赖浏览器660秒。风险低。
...
**B1（最高优先）固定旁白触发加"交接方向与主体"校验。** 反例：2.141的r31，玩家输入是"伸手拿起铭牌"，旁白写成小葵"接过了旧铭牌"，该句又被当作触发证据接受（自证触发）。对照正例：合成用例`photo-catgirl-receives`（玩家"把旧照片递给你"、候选写"接过旧照片"）期望交付且当前通过。两者差别**不在"证据是否来自候选正文"，而在交接方向是否与玩家输入一致**——因此改法不是排除候选来源（那会破坏正例），而是：以玩家输入解析出的持有／交接方向为准，只有"玩家递出→猫娘接收"或"猫娘此前已持有"才认可"猫娘接触／接过"类条件；方向不符时本轮不交付并保留待交付状态。实现位置：`numeric_v2_fixed_narration.review_candidates` 的复核要求与 `apply_triggers` 的来源分级。风险：可能降低交付率，需同时观测"提前交付"与"漏交付"。验证：r31冻结为反例（应不交付），`photo-catgirl-receives`保留为正例（应交付）。
