...
自由输入不增加前端模式开关或关键词分类器。现有 Evaluator 在判定本轮隐藏数值与待确认提议态度时，同时返回一个不持久化的交互意图：`chat`、`scene_action` 或 `mixed_or_unclear`。纯情绪、关系、玩笑和主观交流属于 `chat`；具体行动、决定，以及必须读取环境、设备、物品、路线、风险或物理状态才能回答的问题属于 `scene_action`；两者并存或证据不足时保守使用 `mixed_or_unclear`。

该投影只控制 Actor 本轮的回应优先级。`chat` 要完整回应玩家，不受超推荐回合的强收束提示驱动，不读取下一幕导演方向，也不在正文重述待确认转场；`scene_action` 要直接交付当前幕和作者事实范围内的可见结果，事实不足时明确保持未知；混合态先回应对白或情绪，再处理玩家已经明确实施的行动，不能把讨论升级成执行。标签不会进入 Session、Ledger 或 Runtime，不参与数值、路线和换幕；待确认转场仍只使用 `accept / reject / unclear`。

该设计不限制玩家自由输入。Evaluator 输出缺失或服务降级时使用 `mixed_or_unclear`，从而保留对白与行动两部分；任何分类都不能拒绝、改写玩家消息。生产链路复用已有 Evaluator 请求，没有新增模型调用。压测报告只汇总匿名标签计数，用于发现“设备问句被误判为闲聊”一类问题，不记录额外玩家内容。

长时间纯闲聊允许情节暂不推进，但仍保留既有事实、人格和关系边界。若四次正文改写都因语义趋同被重复保护拒绝，最后一稿仅在无数值变化、无场景更新、无新转场且正文较短时可降级提交；事实边界、玩家行动所有权和正式转场复核不因此放宽。这个降级优先保证对话可用性，不把纯闲聊伪装成新剧情事实。
